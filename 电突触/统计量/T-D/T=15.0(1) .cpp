...
// 延迟系数统计量(同步因子) （此系统在温度在大于24左右会变为静息态，故温度只考虑到24）
// 使用的是无标度网络
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
double k = 0.0; // 电磁反馈增益
double step = 0.01;
double v_tau[NN + 1][8300] = {0.0}; // 数组太大无法定义局部变量，定义全局变量使用

int main() {
	fp1 = fopen("R_D(0.011-0.153)_D_xi=0.1_T=15.0_I=20.dat", "wb");
	run_one();
	fclose(fp1);
