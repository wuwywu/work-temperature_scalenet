...
double k=0.0; // 电磁反馈增益 
double step=0.01;
double v_tau[NN+1][2030]={0.0}; // 数组太大无法定义局部变量，定义全局变量使用
 
int main(){
	fp1=fopen("T-R化学_D=0.1_D_xi=0.1_tau=20.0_I=20(1000).dat","w");
	run_one();
	fclose(fp1);
...
void run_one(){
	...
	int a_t;
	int v_i,v_j;            // 神经元循环 
	double D=0.1,Isyn,alpha_t[NN+1];  // 同步(化学耦合)
	int tau=2000;           // 时间延迟
	// 放电时间
	int flag1[NN+1]={0}; 
...
//	int i, j;      // 循环使用
	fp_SF=fopen("scalefree_matrix1000.dat","rb");	
	for(i=1; i<N_sf+1; i++){
		for(j=1; j<N_sf+1; j++){
			fscanf(fp_SF,"%d %d %d\n",&i_sf, &j_sf, &L_sf);
//			printf("%d %d %d\n", i_sf, j_sf, L_sf);
