//wuyong @ccnu.cn 2022.09.13 // wuyong@mails.ccnu.edu.cn
// 延迟与温度系数统计量 （同步因子） （此系统在温度在大于24左右会变为静息态，故温度只考虑到24）
// 使用的是无标度网络
#include <stdlib.h>
#include <stdio.h>
#include <math.h>
...
int main() {
	fp1 = fopen("D(0.0781-0.0805)-T(0-24)-delta1_tau=0.0_D_xi=0.1_I=20.dat", "w");
	run_one();
	fclose(fp1);
...
void run_one() {
	...
	int a_t, a_t1;
	int v_i, v_j;           // 神经元循环
	double D = 0.0781, Isyn;   // 同步
	int tau = 0, ktau = 0;         // 时间延迟
				...
				for (v_i = 1; v_i < NN + 1; v_i++) {
					...
					if (i >= 10000) {
						for (v_j = 1; v_j < NN + 1; v_j++) {	// 循环连接矩阵（同步）
							Isyn = Isyn + D * edge[v_i][v_j] * (v_tau[v_j][ktau] - v0[v_i]); // 同步
						}
					}
...
double fv(double v, double m, double n, double h, double e) {
	...
	double result;
	result = (-36.0 * n * n * n * n * (v + 77.0) - 120.0 * m * m * m * h * (v - 50.0) - 0.3 * (v + 54.4) - k *
	          (0.4 + 3.0 * 0.02 * e * e) * v) / cm;
	return result;
}

double gn(double v, double n, double T) {
	double result;
	result = 0.01 * (v + 55.0) * pow(3, (T - 6.3) / 10) / (1 - exp(-(v + 55) / 10)) * (1 - n) - 0.125 * exp(-
	         (v + 65.0) / 80) * pow(3, (T - 6.3) / 10) * n;
	return result;
}

double gm(double v, double m, double T) {
	double result;
	result = 0.1 * pow(3, (T - 6.3) / 10) * (v + 40.0) / (1.0 - exp(-(v + 40) / 10.0)) * (1.0 - m) - 4.0 * pow(3,
	         (T - 6.3) / 10) * exp(-(v + 65.0) / 18.0) * m;
	return result;
}

double gh(double v, double h, double T) {
	double result;
	result = 0.07 * pow(3, (T - 6.3) / 10) * exp(-(v + 65.0) / 20.0) * (1.0 - h) - 1.0 * pow(3,
	         (T - 6.3) / 10) / (1.0 + exp(-(v + 35.0) / 10.0)) * h;
	return result;
}
