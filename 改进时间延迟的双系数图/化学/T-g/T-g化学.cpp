...
int main() {
	fp1 = fopen("D(0.1141-0.1195)-T(0-24)-delta化学_tau=0.0_D_xi=0.1_I=20.dat", "w");
	run_one();
	fclose(fp1);
...
void run_one() {
	...
	int a_t, a_t1;
	int v_i, v_j;           // 神经元循环
	double D = 0.1141, Isyn, alpha_t[NN + 1]; // 同步
				...
				for (v_i = 1; v_i < NN + 1; v_i++) {
					...
					if (i >= 10000) {
						for (v_j = 1; v_j < NN + 1; v_j++) {	// 循环连接矩阵（同步）
							Isyn = Isyn + D * edge[v_i][v_j] * v_tau[v_j][ktau] * (0.0 - v0[v_i]); // 同步(化学耦合)
						}
					}
