...
// 化学突触 （此系统在温度在大于24左右会变为静息态，故温度只考虑到24）
// 单系数统计量（同步因子）
// 使用的是无标度网络
#include <stdlib.h>
#include <stdio.h>
...
double k = 0.0; // 电磁反馈增益
double step = 0.01;
double v_tau[NN + 1][8100] = {0.0}; // 数组太大无法定义局部变量，定义全局变量使用

int main() {
	fp1 = fopen("R_D(0.011-0.153)_D_xi=0.1_T=10.0_I=20(化学).dat", "w");
	run_one();
	fclose(fp1);
...
void run_one() {
	...
	double v0[NN + 1], m0[NN + 1], n0[NN + 1], h0[NN + 1], e0[NN + 1], time = -200.0; // 初始化200 // 创建变量
	int i, j, N_time = 300000; // 循环使用
	double Iext = 20.0;     // 外部刺激电流
	double T = 10.0;        // 温度
	int v_i, v_j;           // 神经元循环
	double D = 0.011, Isyn, alpha_t[NN + 1]; // 同步(化学耦合)
	int a_D;
	int tau = 0;         // 时间延迟
	// 放电时间
	int flag1[NN + 1] = {0};
