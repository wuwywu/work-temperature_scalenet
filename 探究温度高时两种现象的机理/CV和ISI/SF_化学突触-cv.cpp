...
double k=0.0; // 电磁反馈增益 
double step=0.01;
double v_tau[NN+1][4100]={0.0}; // 数组太大无法定义局部变量，定义全局变量使用
 
int main(){
//	fp1=fopen("v_t_D=0.1_D_xi=0.1__tau=23_T=5.3.dat","w");
//	fp2=fopen("放电点图化学(2000-2100)_D=0.01_D_xi=0.1__tau=0_T=10.dat","w");
//	fp3=fopen("膜电位（化学）_D=0.1_D_xi=0.1__tau=30_T=11.0.dat","w");
	fp4=fopen("CV_ISI（化学）_D=0.1_D_xi=0.1__tau=30_T=15.0-24.dat","w");	
	run_one();
//	fclose(fp1);
//	fclose(fp2);
//	fclose(fp3);
...
void run_one(){
	...
	int a_t;
	int v_i,v_j;            // 神经元循环 
	double D=0.01,Isyn,alpha_t[NN+1];  // 同步(化学耦合)
	int tau=3000;           // 时间延迟
	// 放电时间
	int flag1[NN+1]={0}; 
...
//		printf("开始\n");
	...
	//		}
		...
		printf("%f %f %f %f %f %f\n", T, CV_ave, CV_SF[2], CV_SF[100], average[2], average[100]);	
		fprintf(fp4,"%f %f %f %f %f %f\n", T, CV_ave, CV_SF[2], CV_SF[100], average[2], average[100]); // 存储统计量
